...
const asyncTemplate = recipePromise =>
  html` ${until(recipePromise, recipeSkeleton())}`;
...
const detailsTemplate = (
  ...
  <div>
    ${canLike
      ? html`<a href="javascript:void(0)" @click=${onLike}>Like</a>`
      : nothing}
...
export function showDetails(ctx) {
  ...
  ctx.render(
    asyncTemplate(
      loadRecipe(id, userId, onLike, createSubmitHandler(onComment))
    )
  );
